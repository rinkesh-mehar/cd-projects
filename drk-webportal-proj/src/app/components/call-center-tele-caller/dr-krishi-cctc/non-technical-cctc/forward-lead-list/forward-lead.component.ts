...
@Component({
  selector:'app-forward-lead',
  templateUrl:'./forward-lead.component.html',
  styleUrls: ['./forward-lead.component.less']
})
export class ForwardleadComponent implements OnInit{
  ...
  onSave() {
    ...
    if (!this.farmerCalling.invalid) {
      ...
      if (this.farmerCalling.value['callingstatus'] == '10') {
        this.router.navigate(['/dr-krishi-non-technical-forward-lead-form/' + this.farmerCalling.value.taskid])
        return
      }
  ...
  public getCCTCNonTechnicalList(){
    this.dtOptions = {
      ...
      ajax: (parameters: any, callback) => {
        ...
        this.drKrishiNonTechnicalService.getLeadCallingListForForward(this.filterValues.areaId, this.filterValues.commodityId, this.filterValues.stateId, this.filterValues.regionId, this.filterValues.districtId, this.filterValues.villageId, parameters).subscribe(resp => {
            if(resp.status == 200){
  ...
  modalClose(){
    ...
    getCommodities() {
          this.drKrishiNonTechnicalService.getCCTCNonTechnicalCommodity().subscribe(
            (data) => {
              if(data.status = 200){
