...
import { callingStatusModel, filterValueModel, ResponseMessage, SubmitNontechincalCallingStatus } from '../non-technical-list/dr-krishi-cctc-non-technical.model';
import { DeliveryConfirmation } from './delivery-confirmation.model';
...
@Component({
      ...
      styleUrls: ['./delivery-confirmation.component.less']
})
export class DeliveryconfirmationComponent implements OnInit{
  ...
  public non_technical_list: DeliveryConfirmation[];
  public non_technical_list_temp = [] as DeliveryConfirmation[];
  ...
  onSave() {
    ...
    if (!this.farmerCalling.invalid) {
      ...
      if (this.farmerCalling.value['callingstatus'] == '10') {
        this.router.navigate(['/dr-krishi-non-technical-information-deliver-confirm-form/' + this.farmerCalling.value.taskid])
        return
      }

      this.deliveryConfirmationService.submitUserGI(this.callingStatusOption).subscribe(
  ...
  public getCCTCNonTechnicalList(){
    this.dtOptions = {
      ...
      ajax: (parameters: any, callback) => {
        ...
        this.deliveryConfirmationService.getLeadCallingListForSpot(this.filterValues.stateId, this.filterValues.regionId, this.filterValues.districtId, this.filterValues.villageId, parameters).subscribe(resp => {
            if(resp.status == 200){
              // TODO - Please remove this after discussion of implementation
  ...
  getStates(){
    this.deliveryConfirmationService.getCCTCNonTechnicalState().subscribe(
      (data) => {
        if(data.status = 200){
...
searchRegionData(){
  ...
  this.deliveryConfirmationService.getCCTCNonTechnicalRegion(this.state).subscribe(
    (data) => {
      if(data.status = 200){
  ...
  searchDistrictsData() {
      this.deliveryConfirmationService.getCCTCNonTechnicalDistricts(this.region).subscribe(
        (data) => {
          if(data.status = 200){
    ...
    searchVillagesData() {
      this.deliveryConfirmationService.getCCTCNonTechnicalVillages(this.district).subscribe(
        (data) => {
          if(data.status = 200){
  ...
  // added for calling status list - Pranay
  getCallingStatus() {
    this.deliveryConfirmationService.getCallingStatusList().subscribe(
      (data) => {
        if(data.status = 200) {
