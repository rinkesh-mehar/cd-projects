...
import {
  callingStatusModel,
  DrKrishiNonTechnical, filterValueModel, ResponseMessage,
  SubmitNontechincalCallingStatus
} from "../non-technical-list/dr-krishi-cctc-non-technical.model";
...
import {AgriotaStatusListService} from "./agriota-status-list.service";
import {AgriotaStatus} from "./agriota-status.model";

@Component({
      selector:'app-agriota-status',
      templateUrl:'./agriota-status.component.html',
      styleUrls: ['./agriota-status.component.less']
})
export class AgriotastatusComponent implements OnInit{
  @ViewChild(DataTableDirective) datatableElement: DataTableDirective;
  public dtOptions: DataTables.Settings = {};
  public dtTrigger: Subject<any> = new Subject();
  public non_technical_list: AgriotaStatus[];
  public non_technical_list_temp = [] as AgriotaStatus[];
  ...
  public datatableemptymessage: string;
      ...
      constructor(private modalService: BsModalService,
                  ...
                  private agriotaStatusListService: AgriotaStatusListService,
  ...
  onSave() {
    ...
    if (!this.farmerCalling.invalid) {
      ...
      if (this.farmerCalling.value['callingstatus'] == '10') {
        this.router.navigate(['/dr-krishi-non-technical-information-agriota-status-form/' + this.farmerCalling.value.taskid])
        return
      }

      this.agriotaStatusListService.submitUserGI(this.callingStatusOption).subscribe(
        (data: ResponseMessage) => {
          if (data.status == 200) {
            const initialState = {
              title: "Success",
              content: 'The calling status has been updated',
              action: "/dr-krishi-non-technical-agriota-status-list"
            };
  ...
  public getCCTCNonTechnicalList(){
    this.dtOptions = {
      ...
      ajax: (parameters: any, callback) => {
        ...
        this.agriotaStatusListService.getLeadCallingListForSpot(this.filterValues.stateId, this.filterValues.regionId, this.filterValues.districtId, this.filterValues.villageId, parameters).subscribe(resp => {
          if(resp.status == 200){
            // this.non_technical_list = resp.data.data;
            this.non_technical_list = this.non_technical_list_temp;
            console.log('this.non_technical_list is --------------->', this.non_technical_list);
  ...
  getStates(){
    this.agriotaStatusListService.getCCTCNonTechnicalState().subscribe(
      (data) => {
        if(data.status = 200){
  ...
  searchRegionData(){
    ...
    this.agriotaStatusListService.getCCTCNonTechnicalRegion(this.state).subscribe(
      (data) => {
        if(data.status = 200){
  ...
  searchDistrictsData() {
    this.agriotaStatusListService.getCCTCNonTechnicalDistricts(this.region).subscribe(
      (data) => {
        if(data.status = 200){
  ...
  searchVillagesData() {
    this.agriotaStatusListService.getCCTCNonTechnicalVillages(this.district).subscribe(
      (data) => {
        if(data.status = 200){
