...
import { DrKrishiNonTechnical, callingStatusModel, ResponseMessage, SubmitNontechincalCallingStatus, filterValueModel } from './dr-krishi-cctc-non-technical.model';
import { DrKrishiNonTechnicalService } from './dr-krishi-non-technical.service';
...
@Component({
  selector: 'app-non-technical-list',
  templateUrl: './non-technical-list.component.html',
  styleUrls: ['./non-technical-list.component.less']
})
export class DrKrishiNonTechnicalListComponent implements OnInit {
  ...
  onSave() {
    ...
    if (!this.farmerCalling.invalid) {
      ...
      if (this.farmerCalling.value['callingstatus'] == '10') {
        this.router.navigate(['/dr-krishi-non-technical-information-form/' + this.farmerCalling.value.taskid])
        return
      }
  ...
  public getCCTCNonTechnicalList(){
    this.dtOptions = {
      ...
      ajax: (parameters: any, callback) => {
        ...
        this.drKrishiNonTechnicalService.getLeadCallingListForForward(this.filterValues.areaId,0, this.filterValues.stateId, this.filterValues.regionId, this.filterValues.districtId, this.filterValues.villageId, parameters).subscribe(resp => {
            if(resp.status == 200){
              ...
              console.log('filtered non_technical_list ', this.non_technical_list);
      ...
      columns: [
        { data: 'serialNumber', searchable: false }, { data: 'farmerName',  searchable: true}, { data: 'stateName',  searchable: true}, { data: 'regionName',  searchable: true}, { data: 'districtName',  searchable: true}, { data: 'villageName',  searchable: true}, { data: 'type',  searchable: true },
...
searchRegionData(){
  ...
  searchDistrictsData() {
    ...
    searchVillagesData() {
      this.drKrishiNonTechnicalService.getCCTCNonTechnicalVillages(this.district).subscribe(
        (data) => {
          if(data.status = 200){
