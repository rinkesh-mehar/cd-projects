...
import { FormResponse, IncomingCallsForm, MasterDataList, SubmitNontechincal } from './incoming-calls-form.model';
import { IncomingCallsFormService } from './incoming-calls-form.service';
@Component({
      selector:'app-incoming-calls-form',
      templateUrl:'./incoming-calls-form.component.html',
      styleUrls: ['./incoming-calls-form.component.less']
})
export class IncomingcallsformComponent implements OnInit{
      ...
      constructor(
            ...
            private incomingCallsFormService: IncomingCallsFormService,
          ) {
  ...
  submitted = false;
  public information: IncomingCallsForm;
...
public DrKrishiNonNonTechnicalInformationFormService() {
      ...
      this.incomingCallsFormService.getDrKrishiNonTechnicalSpotFormService(this.taskId).subscribe(
        ...
        (data: FormResponse) => {
          if (data.status == 200) {
            ...
            const initialState = {
              ...
              action: '/dr-krishi-non-technical-information-incoming-calls-form'
            };
          ...
          const initialState = {
            ...
            action: '/dr-krishi-non-technical-information-incoming-calls-form/' + this.taskId
          };
          this.modalRef = this.modalService.show(SuccessModalComponent, { initialState, backdrop: 'static', keyboard: false });
          return;
        });
      ...
      if (!this.informationForm.invalid) {
        ...
        if (this.information.primarymobilemo === this.information.alternatemobileno) {
          ...
          if (this.information.masterData.farmerCropInfoList.length == 0) {
            ...
            this.incomingCallsFormService.submitDrKrishiNonTechnicalInformationForm(this.submitinformation).subscribe(
              (data) => {
                ...
                if (data.status == 200) {
                  const initialState = {
                    ...
                    action: '/dr-krishi-non-technical-incoming-calls-list'
                  };
                  this.modalRef = this.modalService.show(SuccessModalComponent, {initialState, backdrop: 'static', keyboard: false});
                  return;
                } else {
    ...
    onSaveFarmerCropInfo() {
     ...
     if (this.checkExistFarmerCrop()){
      ...
      this.incomingCallsFormService.saveAndUpdateFarmerCropInfo(this.farmerCrop.value).subscribe(
        (data) => {
    ...
    public onEditFarmerCropInfo(farmercrops: TemplateRef<any>, farmerCropId: string) {
      this.addEditFlsg = true;
      this.incomingCallsFormService.getFarmerCropInfo(farmerCropId).subscribe(
        (data) => {
          ...
          if (data.status == 200) {
            ...
            this.incomingCallsFormService.getVarietyList(this.commodityId).subscribe(
              (data) => {
                if (data.status = 200) {
                  this.varietyList = data.data;
                }
                this.incomingCallsFormService.geFarmerCropListByTaskId(this.taskId).subscribe(
                  (data) => {
                    if (data.status = 200) {
                      this.existingFarmerCropInfo = data.data;
    ...
    public onDeleteFarmerCropInfo(cropInfoId: string) {
      this.addEditFlsg = true;
      this.incomingCallsFormService.deleteFarmerCropInfo(cropInfoId).subscribe(
        (data) => {
