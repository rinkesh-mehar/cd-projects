...
@Component({
  selector: 'app-spot-lead-form',
  templateUrl: './spot-lead-form.component.html',
  styleUrls: ['./spot-lead-form.component.less']
})
export class SpotleadformComponent implements OnInit{
  ...
  constructor(
    ...
    this.farmerCrop = this.formBuilder.group({
      ...
      hasIrrigation: [-1],
      croppingArea: [0.00],
      sellerGivenQtyTon: ['', [Validators.required]],
      dateOfAvailability: ['', [Validators.required]],
  ...
  onSubmit() {
    ...
    if (this.addEditFlag || this.editFlag) {
  ...
  onSaveFarmerCropInfo() {
    ...
    // this.farmerCrop.get('taskId').setValue(this.taskId);
  ...
  public onCall(farmercrops: TemplateRef<any>) {
    ...
    this.clearValidatorOfFarmerCrop();
    ...
    this.farmerCrop.markAsUntouched();
    return;
  }
...
// Based on action check and uncheck record - CDT-Ujwal
    ...
    if (this.cropType == 4) {
      ...
      for (let i of this.information.masterData.farmerCropInfoList/*.filter(a => {
        ...
        if ((this.flag == false && k != this.editNumber) || (this.flag != false)) {
          if (i.commodityId == this.commodityId && i.varietyId == this.varietyId && i.cropTypeId == this.cropType) {
            return isFound = true;
          }
        }
        k++;
      ...
      for (let i of this.information.masterData.farmerCropInfoList/*.filter(a => {
        ...
        if ((this.flag == false && k != this.editNumber) || (this.flag != false)) {
          if (i.commodityId == this.commodityId && i.varietyId == this.varietyId && i.sowingDate == this.dateServiceService.convertLongDateToShort(this.sowingDate)  && i.croppingArea == this.croppingArea && i.cropTypeId == this.cropType) {
            return isFound = true;
          }
        }
        k++;
  ...
  private clearValidatorOfFarmerCrop() {
    ...
    this.farmerCrop.get('yield').clearValidators();
    this.farmerCrop.get('sellerGivenQtyTon').clearValidators();
    this.farmerCrop.get('dateOfAvailability').clearValidators();
    this.farmerCrop.get('leadResponse').clearValidators();
    this.farmerCrop.get('sowingDate').clearValidators();

  }

  private setValidatorOfFarmerCrop() {
    this.farmerCrop.get('cropType').setValidators([Validators.required]);
    ...
    if (this.cropType == 4){
      ...
      this.farmerCrop.get('sellerGivenQtyTon').setValidators([Validators.required, Validators.pattern('^-?[1-9]\\d*(\\.\\d{1,2})?$')]);
      this.farmerCrop.get('dateOfAvailability').setValidators([Validators.required]);
