...
import { callingStatusModel, filterValueModel, SubmitNontechincalCallingStatus } from '../non-technical-list/dr-krishi-cctc-non-technical.model';
import { IncomingCalls, ResponseMessage } from './incoming-calls.model';
...
@Component({
      ...
      styleUrls: ['./incoming-calls.component.less']
})
export class IncomingcallsComponent implements OnInit{
      ...
      public non_technical_list: IncomingCalls[];
      public non_technical_list_temp = [] as IncomingCalls[];
      ...
      onSave() {
        ...
        if (!this.farmerCalling.invalid) {
          ...
          if (this.farmerCalling.value['callingstatus'] == '10') {
            this.router.navigate(['/dr-krishi-non-technical-information-incoming-calls-form/' + this.farmerCalling.value.taskid])
            return
          }

          this.incomingCallsService.submitUserGI(this.callingStatusOption).subscribe(
      ...
      public getCCTCNonTechnicalList(){
        this.dtOptions = {
          ...
          ajax: (parameters: any, callback) => {
            ...
            this.incomingCallsService.getLeadCallingListForSpot(this.filterValues.stateId, this.filterValues.regionId, this.filterValues.districtId, this.filterValues.villageId, parameters).subscribe(resp => {
                if(resp.status == 200){
                  // TODO - Please remove this after discussion of implementation
      ...
      getStates(){
        this.incomingCallsService.getCCTCNonTechnicalState().subscribe(
          (data) => {
            if(data.status = 200){
    ...
    searchRegionData(){
      ...
      this.incomingCallsService.getCCTCNonTechnicalRegion(this.state).subscribe(
        (data) => {
          if(data.status = 200){
      ...
      searchDistrictsData() {
          this.incomingCallsService.getCCTCNonTechnicalDistricts(this.region).subscribe(
            (data) => {
              if(data.status = 200){
        ...
        searchVillagesData() {
          this.incomingCallsService.getCCTCNonTechnicalVillages(this.district).subscribe(
            (data) => {
              if(data.status = 200){
      ...
      // added for calling status list - Pranay
      getCallingStatus() {
        this.incomingCallsService.getCallingStatusList().subscribe(
          (data) => {
            if(data.status = 200) {
