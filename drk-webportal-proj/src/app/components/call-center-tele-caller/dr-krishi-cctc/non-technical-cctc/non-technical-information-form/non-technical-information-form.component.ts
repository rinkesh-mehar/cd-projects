...
import { DRKrishiNonTechnicalInformationForm, MasterDataList, List, FormResponse, SubmitNontechincal, FarmerCropInfo, CropResponse } from './non-technical-information.model';
import { DrKrishiNonNonTechnicalInformationFormService } from './non-technical-information.service';
...
@Component({
  selector: 'app-non-technical-information-form',
  templateUrl: './non-technical-information-form.component.html',
  styleUrls: ['./non-technical-information-form.component.less']
})
export class NonTechnicalInformationFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.farmerCrop = this.formBuilder.group({
      ...
      hasIrrigation: ['', [Validators.required]],
      croppingArea: ['', [Validators.required, Validators.pattern('^-?[1-9]\\d*(\\.\\d{1,2})?$')]],
      sellerGivenQtyTon: [''],
      dateOfAvailability: [''],
      ...
      (data: FormResponse) => {
        if (data.status == 200) {
          ...
          const initialState = {
            ...
            action: '/dr-krishi-non-technical-information-form'
          };
        ...
        const initialState = {
          ...
          action: '/dr-krishi-non-technical-information-form/' + this.taskId
        };
        this.modalRef = this.modalService.show(SuccessModalComponent, { initialState, backdrop: 'static', keyboard: false });
        return;
      });
...
// Based on action check and uncheck record - CDT-Ujwal
  ...
  checkExistFarmerCrop(): boolean {
    ...
    for (let i of this.information.masterData.farmerCropInfoList.filter(a => {
      ...
      if (i.commodityId == this.commodityId && i.varietyId == this.varietyId && i.sowing == this.dateServiceService.convertLongDateToShort(this.sowing) && i.croppingArea == this.croppingArea) {
        return isFound = true;
      }
    }
    return isFound;
