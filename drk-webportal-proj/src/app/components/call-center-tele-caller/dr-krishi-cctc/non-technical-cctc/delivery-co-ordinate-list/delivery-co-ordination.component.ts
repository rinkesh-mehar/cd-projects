...
import { callingStatusModel, DeliveryCoordination, filterValueModel, ResponseMessage, SubmitNontechincalCallingStatus } from "./delivery-co-ordination.model";
import { DeliveryCoordinationService } from "./delivery-co-ordination.service";
import {DeliveryConfirmation} from '../delivery-confirmation-list/delivery-confirmation.model';

@Component({
      selector:'app-delivery-co-ordination',
      templateUrl:'./delivery-co-ordination.component.html',
      styleUrls: ['./delivery-co-ordination.component.less']
})
export class DeliverycordinationComponent implements OnInit{

  @ViewChild(DataTableDirective) datatableElement: DataTableDirective;
  public dtOptions: DataTables.Settings = {};
  public dtTrigger: Subject<any> = new Subject();
  public non_technical_list: DeliveryCoordination[];
  public non_technical_list_temp = [] as DeliveryCoordination[];
  ...
  constructor(
    ...
    private deliveryCoordinationService: DeliveryCoordinationService,
  ...
  onSave() {
    ...
    if (!this.farmerCalling.invalid) {
      ...
      if (this.farmerCalling.value['callingstatus'] == '10') {
        this.router.navigate(['/dr-krishi-non-technical-information-delivery-co-ordinat-form/' + this.farmerCalling.value.taskid])
        return
      }

      this.deliveryCoordinationService.submitUserGI(this.callingStatusOption).subscribe(
        (data: ResponseMessage) => {
          if (data.status == 200) {
            const initialState = {
              title: "Success",
              content: 'The calling status has been updated',
              action: "/dr-krishi-non-technical-delivery-co-ordinat"
            };
  ...
  public getCCTCNonTechnicalList(){
    this.dtOptions = {
      ...
      ajax: (parameters: any, callback) => {
        ...
        this.deliveryCoordinationService.getLeadCallingListForSpot(this.filterValues.stateId, this.filterValues.regionId, this.filterValues.districtId, this.filterValues.villageId, parameters).subscribe(resp => {
            if(resp.status == 200){
              // TODO - Please remove this after discussion of implementation
  ...
  getStates(){
    this.deliveryCoordinationService.getCCTCNonTechnicalState().subscribe(
      (data) => {
        if(data.status = 200){
...
searchRegionData(){
  ...
  this.deliveryCoordinationService.getCCTCNonTechnicalRegion(this.state).subscribe(
    (data) => {
      if(data.status = 200){
  ...
  searchDistrictsData() {
      this.deliveryCoordinationService.getCCTCNonTechnicalDistricts(this.region).subscribe(
        (data) => {
          if(data.status = 200){
    ...
    searchVillagesData() {
      this.deliveryCoordinationService.getCCTCNonTechnicalVillages(this.district).subscribe(
        (data) => {
          if(data.status = 200){
  ...
  // added for calling status list - Pranay
  getCallingStatus() {
    this.deliveryCoordinationService.getCallingStatusList().subscribe(
      (data) => {
        if(data.status = 200) {
