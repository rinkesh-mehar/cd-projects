import {Component, OnInit, ViewChild} from '@angular/core';
import {SuccessModalComponent} from '../../global/success-modal/success-modal.component';
import {ErrorModalComponent} from '../../global/error-modal/error-modal.component';
import {UserRightsService} from '../../services/user-rights.service';
import {Router} from '@angular/router';
import {GeoDistrictService} from '../services/geo-district.service';
import {GeoStateService} from '../services/geo-state.service';
import {DistrictAlias} from './model/DistrictAlias';
import {StoreService} from './AliasService/StoreService';
import { Sort } from '@angular/material';
declare var $: any;
@Component({
    selector: 'app-district-alias',
    templateUrl: './add-geo-district-alias.component.html',
    styleUrls: ['./add-geo-district-alias.component.css']
})
export class AddGeoDistrictAliasComponent implements OnInit {

    @ViewChild('successModal') public successModal: SuccessModalComponent;
    @ViewChild('errorModal') public errorModal: ErrorModalComponent;

    isSubmitted: boolean = false;
    stateList: any = [];
    aliasList: any = [];
    html: any;
    disableRow: boolean = false;
    districtList: any = [];
    isDistrict: boolean = true;
    isAddNewRow: boolean = false;
    row = [
        {
            id: '',
            name: '',
            email: ''
        }
        ];
    aliasName: any;
    indexId: any;
    pageDistrictAlias: DistrictAlias;
    selectedPage: number;
    maxSize = 10;
    getDistricts: any= [];
    getCurrentState: any;


    ngOnInit(): void {
        this.disableRow = false;
        this.isDistrict = true;
        this.isAddNewRow = false;
        this.getListOfDistrictAlias(0);
        this.row = [];
    }

    constructor(public userRightsService: UserRightsService, public geoStateService: GeoStateService,
                public router: Router, public geoDistrictService: GeoDistrictService,
                public storeDistrictService: StoreService) {
    }

    addNewRow() {
        this.disableRow = true;
        this.loadAllState();
        this.isDistrict = false;
        this.isAddNewRow = true;
        const obj = {
            id: '',
            name: '',
            email: ''
        };
        this.aliasName ='';
        this.row.push(obj);
        // this.service.success('New row added successfully', 'New Row');

        return true;
    }

    deleteRow(x) {

        this.disableRow = false;
        if ( x >= 0) {
            this.row.splice(x, 1);
        } else {
           this.getListOfDistrictAlias(0);

        }

    }
    onSelect(page: number): void {
        console.log("selected page : " + page);
        this.selectedPage = page;
        this.getListOfDistrictAlias(page);
    }

    getListOfDistrictAlias(page: number) {
        this.isDistrict = true;
        return this.geoDistrictService.getPageGeoDistrictAlias(page).subscribe(
            page=>{
                this.pageDistrictAlias = page;
                this.getDistricts = page[0].Districts;
            } );

        this.storeDistrictService.addToStore(this.pageDistrictAlias.Districts);
    }


    getData(id,event){
        if(event.target.value){
            return
        }
        var result = this.getDistricts.filter(function (v, i) {
            return v.stateCode == id
        })  
        var data = `<option value=" ">--Select District--</option>`;
        $.each(result, function(index, value) {
            data += `<option value="${value.districtCode}">${value.name}</option>`
        })
        $(event.target).html(data);
        console.log('sadasdoj')

    }


    

    saveAliasCommodity(aliasId, stateCode, index, event, validAlias) {
        let data: any;
        let code;
        let aliasName:string;
        this.html = $(event.target);
        if (validAlias == 'addAlias') {
            aliasName = $(event.target).closest('tr').find('input').val();
            code = $(event.target).closest('tr').find('.district').val();
            stateCode = $(event.target).closest('tr').find('.state').val();
            if (aliasName == '' || aliasName == null || aliasName.trim() == '') {
                $(event.target).closest('tr').find('span').text(`District Alias Name is required`);
                return;
            }
            aliasName = aliasName.trim();
        }
        
        console.log("aliasName : " + aliasName);
        this.indexId = index;
        console.log('Districts Id ------>' + aliasId);
        console.log('Districts Id ------>' + stateCode);
        if (code == undefined) {

        data = {
            'stateCode': stateCode,
            'aliasId': aliasId,
            'aliasName': aliasName,
            'districtCode': event.target.value
        };
    } else {
            data = {
                'stateCode': stateCode,
                'aliasId': aliasId,
                'aliasName': aliasName,
                'districtCode': code
            };
}
        this.geoDistrictService.saveAliasDistrict(data).subscribe(res => {
            this.isSubmitted = true;
            if (res.success) {
                if (this.indexId >= 0 && this.indexId != null){
                    this.deleteRow(this.indexId);
                } else {
                    if (aliasId == 0){
                        this.successModal.showModal('SUCCESS', res.message, '');
                    }
                    // this.ngOnInit();
                    // console.log(this.html.closest('tr').find('.addAction'));
                    this.html.closest('tr').find('.addAction').html(`<span>Saved</span>`);
                }
            } else {
                this.errorModal.showModal('ERROR', res.error, '');
            }
        });

    }

    loadAllState() {
        this.geoDistrictService.GetAllState().subscribe((data: {}) => {
            // this.stateList = Array.from(data['States']);
            this.stateList = data;
        })
    }

    loadDistrictByStateCode(event): void {
        // const index: number = event.target['selectedIndex'] - 1;
        // // tslint:disable-next-line:triple-equals
        // if (index == -1) {
        //     return;
        // }
        // const stateCode = this.stateList[index].StateCode;
        console.log(event.target.value);
        this.geoDistrictService.GetAllDistrictAliasByStateCode(event.target.value).subscribe(
            (data: {}) => {
                // this.districtList = Array.from(data['Districts']);
                this.districtList = data;
                console.log('District list' + this.districtList);

                console.log(this.html);
            });
    }
    validAliasName(){
        const aliasName = $(event.target).val();

        if (aliasName != ''){
            $(event.target).closest('tr').find('span').text(``);
        }
    }
    modalSuccess($event: any) {
        this.ngOnInit();
    }

    sortData(sort: Sort) {
        const data = this.pageDistrictAlias[1].content.slice();
        if (!sort.active || sort.direction == '') {
          this.pageDistrictAlias[1].content = data;
          return;
        }
      
        function compare(firstValue, secondValue, isAsc: boolean) {
          return (firstValue < secondValue ? -1 : 1) * (isAsc ? 1 : -1);
        }
      
        this.pageDistrictAlias[1].content = data.sort((firstValue, secondValue) => {
          const isAsc = sort.direction == 'asc';
          switch (sort.active) {
            case 'stateName':
                return compare(+firstValue.stateName, +secondValue.stateName, isAsc);
            case 'alias':
              return compare(+firstValue.alias, +secondValue.alias, isAsc);
            default:
              return 0;
          }
        });
      }
}
