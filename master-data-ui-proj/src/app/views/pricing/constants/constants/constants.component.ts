...
export class ConstantsComponent implements OnInit {
    ...
    getConstants(sCode, rId, cId): any {
        ...
        this.pricingMspGroupService.getConstants(sCode, rId, cId, this.flag)
            .subscribe(page => this.stateRegionList = page);
    }
        ...
        if (flag === 2) {
            this.stateRegionList = data.sort((firstValue, secondValue) => {
                ...
                switch (sort.active) {
                    ...
                    case 'marginConstant':
                        return compare(firstValue.marginConstant, secondValue.marginConstant, isAsc);
                    case 'buyerConstant':
                        return compare(firstValue.buyerConstant, secondValue.buyerConstant, isAsc);
                    case 'sellerConstant':
                        return compare(firstValue.sellerConstant, secondValue.sellerConstant, isAsc);
                    case 'isBenchmark':
    ...
    download() {
        this.pricingMspGroupService.exportConstantToExcel(this.stateRegionList[0].stateCode, this.stateRegionList[0].regionID, this.stateRegionList[0].commodityID);
    }
}
