...
/**
 * Repository for fetching PVI related map and analytics data.
 * 
 * @since 1.0
 * @author PranaySK
 */
...
@Repository
public class PviRepository {

	private static final Logger LOGGER = LoggerFactory.getLogger(PviRepository.class);
	...
	public PviRepository(NamedParameterJdbcTemplate namedJdbcTemplate) {
	...
	private AnalyticsData prepareBriefAnalyticsResponse(List<AnalyticsParameters> analyticsDataList) {
		...
		/** setting total average value for PVI */
		aggregate.setLabel("Average PVI");
		aggregate.setValue(studioUtil.doubleToInt(avgPyar) + " %");
	 ...
	 * 
	 * @param zoomLevel        the <b>Zoom Level</b> for which {@link AnalyticsData}
	 *                         data to be fetched
	 * @param coordinateClause the coordinate clause for adding coordinate condition
	 * @param caseClause       the color case clause for adding color in map data
	 * 
	...
	public List<AnalyticsParameters> getAnalyticsParams(final String zoomLevel, final String coordinateClause,
		...
		try {
			...
			String analyticsDataSql = "select SeasonID, SeasonName, Irrigation, NsaTgaPer, AverageLhSize, Production, Pvi,"
					+ caseClause
					+ " CropCount, Crop, CropName from ( select value_risk_sd_.SeasonID, season_.Name as SeasonName,\n"
					+ "AVG( IFNULL(value_risk_sd_.PyarPrimary, IFNULL(value_risk_sd_.PyarSurvey, value_risk_sd_.RrfvSecondary)) ) as Pvi,\n"
			...
			throw new DataNotFoundException(ErrorConstants.DATA_FETCH_ERROR,
					"Data Not Found while fetching PVI Analytics details -> " + ex.getMessage());
		}
	}
	 ...
	 * @author PranaySK
	 */
	private List<AnalyticsParameters> getBriefAnalyticsParams(String zoomLevel, final String caseClause,
			final Integer tileId, final Integer seasonId) {
		try {
			...
			throw new DataNotFoundException(ErrorConstants.DATA_FETCH_ERROR,
					"Data Not Found while fetching PVI Brief Analytics details -> " + ex.getMessage());
		}
	}
